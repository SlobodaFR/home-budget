...
describe('RecurringTransaction', () => {
  ...
  it('rejects an out-of-range day of month', () => {
    expect(() =>
      RecurringTransaction.create({ ...baseProps, dayOfMonth: 32 }),
    ).toThrow();
    expect(() =>
      RecurringTransaction.create({ ...baseProps, dayOfMonth: 0 }),
    ).toThrow();
  });
  ...
  it('projects monthly occurrences on the given day', () => {
    ...
    const occurrences = rt.occurrencesBetween(
      new Date('2026-06-10'),
      new Date('2026-09-30'),
    );
  ...
  it('returns nothing for an inactive recurring transaction', () => {
    ...
    expect(
      rt.occurrencesBetween(new Date('2026-01-01'), new Date('2026-12-31')),
    ).toEqual([]);
  });
});
